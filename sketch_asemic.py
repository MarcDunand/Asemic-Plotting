...
vDiv = 4                            #number of vertical divisions there are on a character
ribProb = 0.2                       #chance of an accent being added in any given spot
ribLen = 0.6                        #how long horizontal strokes are
charHeight = 3                      #maximum height of characters
charHeightNoise = 1.5               #how much the size of letters can vary, 1 is no variation, n >= 1
charLineHeightNoise = 1.1           #how much does the character size along an entire line vary (affects all characters on a line)
wordLenStdv = 7                     #standard deviation of word length
...
wiggleMin, wiggleMax = -0.4, 0.4    #maximum vertical offset of one char from the next
lineStartMin, lineStartMax = 0, 0   #maximum horizontal offset from one line to the next
lineSpacing = 2.3                   #vertical spacing between lines. 1 is no spacing
spaceLen = 1                        #how long spaces between words are
kerning = -0.2                      #how long spaces between characters are
charTilt = 0.3                      #how much each char's vertical lines tilt as a ratio to their height
...
maxCharShrink = 0                   #how much smaller letters get at maximum over the height of the page
...
scribbleChance = 0.2                #chance of starting a feature
...
class AsemicSketch(vsketch.SketchClass):
    ...
    def drawChar(self, vsk: vsketch.Vsketch, charSeed, xPos, yPos, yProg, thischarHeight):
        ...
        if charSeed.query() < 0.5:  #conditionally adds a second slash
            ...
            self.slash(vsk, xPos+ribLen, yPos, thischarHeight, accentS/vDiv, accentE/vDiv, thisCharTilt)
            for p in range(maxh+1):
                if charSeed.query() < ribProb:
                    self.accentLine(vsk, xPos-ribLen, yPos, ribLen, thischarHeight, p/vDiv, thisCharTilt)
            ...
            for p in range(0, maxh+1):
                if charSeed.query() < ribProb:
                    self.accentLine(vsk, xPos, yPos, ribLen, thischarHeight, p/vDiv, thisCharTilt)
